...
<body>
    @php
        ...
        $totalVaccinesAdministered = $data->sum(function($row) {
            return $row->male_vaccinated + $row->female_vaccinated;
        });
        ...
        foreach($data as $row) {
            ...
            $coverage = $estimatedPopulation > 0 ? ($totalVaccinated / $estimatedPopulation) * 100 : 0;
            $totalCoverage += $coverage;
        }
        $averageCoverage = $totalRecipients > 0 ? round($totalCoverage / $totalRecipients) : 0;
        
        // Find lowest stock vaccine (lowest total shots)
        $lowestStockVaccine = $data->sortBy('total_shots')->first();
        
        // Group data by vaccine name for charts
        $vaccineData = $data->groupBy('vaccine_name')->map(function($group) {
            return [
                'total' => $group->sum(function($item) {
                    return $item->male_vaccinated + $item->female_vaccinated;
                }),
                'male' => $group->sum('male_vaccinated'),
                'female' => $group->sum('female_vaccinated'),
                'shots' => $group->sum('total_shots')
            ];
        });
        
        // Group by age group
        $ageGroupData = $data->whereNotNull('age_group')->groupBy('age_group')->map(function($group) {
    ...
    <div class="dashboard-container">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <h3>Total Vaccines Administered</h3>
                <div class="value">{{ number_format($totalVaccinesAdministered) }}</div>
            </div>
            <div class="stat-card">
            ...
            <div class="stat-card">
                <h3>Average Coverage Rate</h3>
                <div class="value">{{ $averageCoverage }}%</div>
            </div>
            <div class="stat-card alert">
                <h3>Lowest Stock Alert</h3>
                <div class="value" style="font-size: 1.5rem;">
                    {{ $lowestStockVaccine ? strtoupper($lowestStockVaccine->vaccine_name) : 'N/A' }}
                </div>
            </div>
        </div>
        ...
        <div class="charts-grid">
            <div class="chart-card">
                <h3>Remaining Vaccine Shots/Vials</h3>
            ...
            <div class="chart-card">
                <h3>Vaccination Distribution by Age Group</h3>
                <div class="chart-container">
            ...
            <div class="chart-card">
                <h3>Coverage Rate per Vaccine</h3>
                <div class="chart-container">
        ...
        <div class="table-card">
            <h3><i class="fas fa-table"></i> Immunization Data Table</h3>
    ...
    <script>
        // Chart data from PHP
        const vaccineLabels = @json($vaccineData->keys());
        const vaccineShots = @json($vaccineData->pluck('shots'));
        const vaccineMale = @json($vaccineData->pluck('male'));
        const vaccineFemale = @json($vaccineData->pluck('female'));
        const ageGroupLabels = @json($ageGroupData->keys());
        const ageGroupValues = @json($ageGroupData->values());
        ...
        new Chart(document.getElementById('remainingChart'), {
            ...
            data: {
                ...
                datasets: [{
                    data: vaccineShots,
                    backgroundColor: colorArray.slice(0, vaccineLabels.length)
                }]
            },
            options: {
                responsive: true,
        ...
        if (ageGroupLabels.length > 0) {
            new Chart(document.getElementById('ageDistributionChart'), {
                ...
                data: {
                    ...
                    datasets: [{
                        data: ageGroupValues,
                        backgroundColor: colorArray.slice(0, ageGroupLabels.length)
                    }]
                },
                options: {
                    responsive: true,
        ...
        new Chart(document.getElementById('coverageChart'), {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: @json($vaccineData->pluck('total')),
                    backgroundColor: colorArray.slice(0, vaccineLabels.length)
                }]
            },
            options: {
