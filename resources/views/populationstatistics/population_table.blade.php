...
<head>
    ...
    <title>Modern population Data Table</title>
...
@if($data->isNotEmpty())
    ...
    <thead>
        <tr style="color: white;">
            ...
            <th onclick="sortTable(1, 'string')">Month & Year <i id="icon-1" class="fas fa-sort sort-icon"></i></th>
    ...
    <tbody style="background-color: white;">
        @foreach($data as $row)
            <tr data-id="{{ $row->id }}">
                ...
                <!-- Changed from year to year_month -->
                <td>{{ \Carbon\Carbon::parse($row->year_month.'-01')->format('F Y') }}</td>
                <td>{{ $row->location }}</td>
                ...
                <td class="no-print">
                    <button class="btn btn-warning btn-sm edit-button" data-bs-toggle="modal"
                        ...
                        data-id="{{ $row->id }}" 
                        data-year_month="{{ $row->year_month }}"
        ...
        if (deleteSelectedBtn) {
            ...
            `;
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <p style="margin-bottom: 2rem; font-size: 1.1rem; color: #ccc; line-height: 1.5;">
                        This action will permanently delete <strong>${selectedIds.length} selected record${selectedIds.length > 1 ? 's' : ''}</strong> from the population table. 
                        <br><br>
            ...
            `;
                ...
                confirmBtn.addEventListener("click", () => {
                    ...
                    fetch("{{ url('/population/delete-selected') }}", {
                            method: "DELETE",
                            headers: {
                        ...
                        .then(data => {
                            ...
                            if (data.success) {
                                ...
                                showModernAlert("❌ Error", data.message ||
                                    "Failed to delete selected records.", "error");
                            }
                        })
                        .catch(error => {
                            confirmOverlay.remove();
                            ...
                            showModernAlert("❌ Error",
                                "Something went wrong while deleting selected records.", "error");
                        });
                });
